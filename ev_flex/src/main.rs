...
fn create_flex_series(demand: &EnergyDemand) -> Option<Vec<AggregationDT>> {
    ...
    let time_to_charge_minutes = (energy_demand / demand.max_charging_power * 60) as i64;
    let one_minute_energy_state_change = demand.max_charging_power / 60;

    let asap_charge_time_end = demand.start + Duration::minutes(time_to_charge_minutes);
    let alap_charge_time_end = demand.end - Duration::minutes(time_to_charge_minutes);
    ...
    // Fill middle area
    for time in MinuteDateRange(asap_charge_time_end, alap_charge_time_end) {
    ...
    // Fill alap line
    for time in MinuteDateRange(alap_charge_time_end, demand.end) {
        aggregation_series.push(AggregationDT {
            min_soe: min_soe_state,
            max_soe: max_soe_state,
            max_charging_power: demand.max_charging_power,
