...
fn generate_calendar(date: &chrono::NaiveDate) {
    ...
    next_line.push_str(std::iter::repeat("    ").take(days_from_monday).collect::<String>().as_str());

    for (day_idx, day) in date.iter_days().take_while(|d| d.month() == date.month()).enumerate() {
